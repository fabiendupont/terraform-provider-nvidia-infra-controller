...
var _ datasource.DataSource = &ExpectedSwitchDataSource{}
var _ datasource.DataSourceWithConfigure = &ExpectedSwitchDataSource{}

func NewExpectedSwitchDataSource() datasource.DataSource {
	return &ExpectedSwitchDataSource{}
}

type ExpectedSwitchDataSource struct {
	client *Client
}
...
func (d *ExpectedSwitchDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *ExpectedSwitchDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *ExpectedSwitchDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data ExpectedSwitchDataSourceModel
	...
	if !data.Id.IsNull() && !data.Id.IsUnknown() && data.Id.ValueString() != "" {
		url := d.client.ResolvePath("/v2/org/{org}/nico/expected-switch/{expectedSwitchId}", map[string]string{"expectedSwitchId": data.Id.ValueString()})
		...
		if result == nil {
			resp.Diagnostics.AddError("Expected Switch not found", "No resource found with the given id.")
			return
		}
		...
		url := d.client.ResolvePath("/v2/org/{org}/nico/expected-switch", map[string]string{})
		items, err := d.client.List(ctx, url, map[string]string{})
		if err != nil {
			resp.Diagnostics.AddError("Error listing Expected Switch", err.Error())
			return
		}
		if len(items) > 0 {
			result := items[0]
