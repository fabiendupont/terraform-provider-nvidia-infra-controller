...
page_title: "nico_spectrumx_partition Data Source - nico"
subcategory: ""
description: |-
...
# nico_spectrumx_partition (Data Source)
...
- `query` (String) Search for matches across all SpectrumX Partitions. Input will be matched against name, description, status and labels fields
- `site_id` (String) Filter Partitions by Site. Repeat the parameter to match multiple Sites.
- `status` (String) Filter Partitions by Status. Repeat the parameter to match multiple Statuses.

### Read-Only

- `created` (String) ISO datetime string for when the SpectrumX Partition was created
- `description` (String) Description of the SpectrumX Partition
...
- `vni` (Number) VXLAN Network Identifier for the Partition. Allocated by the Site when the create request omits it
